...
"""Report portafoglio Swissquote — tutto in un file.
   ...
   Legge dati.json e prezzi2.json; scrive report.html (documento intero),
   app.html (versione a schede per l'artifact), report.png, e riscrive dati.json."""
...
def build(pos, prezzi, oggi, costo_eur=None):
    ...
    def add(v, cat, nome, val, prev, costo, extra=None):
        ...
        r = {'nome': nome, 'val': val, 'prev': prev, 'costo': costo,
             ...
             'inizio': (val / costo - 1) if costo else 0.0}
    ...
    for v, b in B.items():
        for cat, c in b['cat'].items():
            ...
            c['inizio'] = (c['val'] / c['costo'] - 1) if c['costo'] else 0.0
            c['peso'] = c['val'] / b['val'] if b['val'] else 0
        tit = b['val'] - b['val_cash']
        cos = b['costo']
        real = pos['realizzi'].get(v, 0)
        b['tit_val'] = tit; b['tit_costo'] = cos
        b['inizio_tit'] = ((tit + real) / cos - 1) if cos else 0
        cash_cost_loc = b['costo_cash'] / fx(v) if fx(v) else 0
        b['inizio_tot'] = ((b['val'] + real) / (cos + cash_cost_loc) - 1) if (cos + cash_cost_loc) else 0
...
<td colspan="3"></td></tr>''')
        for it in sorted(c['it'], key=lambda x: -x['val'])[:6]:
            ...
            extra = ''
            if it.get('ytm') is not None and cat == 'Obbligazionario': extra = f'<span class="w"> {it["ytm"]*100:.2f}%</span>'
...
<div class="accr"><span>EUR/CHF <b class="num">{1/FX['CHF']:.4f}</b> · EUR/USD <b class="num">{1/FX['USD']:.4f}</b></span>
...
def attribuzione(B, FX, n=3):
    """Chi ha mosso il portafoglio oggi, in euro. Nessuna fonte esterna."""
    r = []
    for v, b in B.items():
...
def attrib_blocco(b, v, n=2):
    ...
    return ' · '.join(f"{breve(nm)[:16]} {p(dd)}%" for _, nm, dd in r[:n])
